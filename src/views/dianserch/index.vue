<template>
  <div class="search_body">
    ...
    <div class="search_result">
      <h3>电影/电视剧/综艺</h3>
      <ul>
        <li v-for="(item,inde) in list" :key="inde">
          <div class="img">
            <img :src="item.img | getimg('128.180')" />
          </div>
          <div class="info">
            <p>
              <span>{{item.nm}}</span>
...
export default {
  ...
  methods: {
    axio() {
      this.$axios.get("/api/searchList?cityId=10&kw=" + this.msg).then(res => {
        if (res.data.msg == "ok" && Object.keys(res.data.data).length != 0) {
